...
def embedding(x,
  ...
  model_dim - an int, so that the output is, shape = [..., model_dim]
  ...
  with tf.variable_scope(name,
    ...
    y = tf.nn.embedding_lookup(lookup_table,
                               x)
    if embed_size != model_dim:
      with tf.variable_scope('projection'):
        proj_W = tf.get_variable('proj_W',
                                 shape = [embed_size, model_dim],
                                 initializer = initializer)
      y = tf.tensordot(y,
                       proj_W,
                       axes = 1)
      return y * (model_dim ** 0.5), [lookup_table, proj_W]
    else:
      return y * (model_dim ** 0.5), [lookup_table, None]
