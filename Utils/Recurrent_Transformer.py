...
class Recurrent_Transformer():
  def __init__(self, arg,
               name = None):
    '''
    a combination of a recurrent encoder and a vanilla Transformer, as suggested in arXiv:1904.03092
    '''
    if name:
      self.name = name
    else:
      self.name = 'Recurrent-Transformer'
    batch_size = 32
    input_sequence_size = 10
    output_sequence_size = 12
    if __name__ != '__main__':
      batch_size = input_sequence_size = output_sequence_size = None
    if not arg.use_decoder:
    ...
    with tf.variable_scope('encoder'):
      ...
      encoder_output, encoder_RNN = self.encoder(encoder_input,
    ...
    with tf.variable_scope('output'):
      if self.arg.use_mos:
        ...
        if arg.use_decoder:
          ...
          self.logits = tf.tensordot((encoder_output + encoder_RNN) / 2,
                                     weights,
                                     axes = 1) + bias
        self.loss_cl = loss.Loss(self.logits,
                                 self.targets,
  ...
  def encoder(self, inputs,
    ...
    with tf.variable_scope('positional_encoding'):
      with tf.variable_scope('encoder'):
        posenc_inputs = self.timing_position(inputs)
    x = posenc_inputs
    recurr_x = inputs
    if self.arg.adaptive_mask:
      self.encoder_l0 = []
    ...
    for layer in range(1,
      ...
      with tf.variable_scope('RNN_layer_{}'.format(layer)):
        ...
        recurr_x += recurr_y
    with tf.variable_scope('output'):
      return utils.layer_norm(x), utils.layer_norm(recurr_x,
                                                   name = 'RNN')
...
def argument():
  ...
  arg.rnn_encoder_layers = 2 # the number of RNN layers used in the encoder
...
if __name__ == '__main__':
  ...
  model = Recurrent_Transformer(arg)